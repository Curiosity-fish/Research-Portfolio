...
const packages = [
	{ directory: "packages/telemetry", name: "@earendil-works/tfa-telemetry" },
	{ directory: "packages/ai", name: "@earendil-works/tfa-ai" },
	{ directory: "packages/tui", name: "@earendil-works/tfa-tui" },
	{ directory: "packages/agent", name: "@earendil-works/tfa-agent-core" },
	{ directory: "packages/protocol", name: "@earendil-works/tfa-protocol" },
	{ directory: "packages/client", name: "@earendil-works/tfa-client" },
	{ directory: "packages/session-backends/sqlite-node", name: "@earendil-works/tfa-session-backend-sqlite-node" },
	{ directory: "packages/coding-agent", name: "@earendil-works/tfa-coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "tfa-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `tfa-${platform}.zip` : `tfa-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "tfa.cmd"))) {
			writeFileSync(join(installDirectory, "tfa.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\tfa.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "tfa.ps1"), '& "$PSScriptRoot/node_modules/.bin/tfa.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "tfa.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\tfa.exe" %*\r\n');
		writeFileSync(join(installDirectory, "tfa.ps1"), '& "$PSScriptRoot/node_modules/.bin/tfa.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "tfa"), join(installDirectory, "tfa"));
}
...
if (rootPackageJson.name !== "tfa-monorepo") {
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `tfa-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "tfa.exe" : "tfa")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "tfa.cmd" : "tfa")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "tfa.cmd" : "tfa")} --help`);
	}
}
