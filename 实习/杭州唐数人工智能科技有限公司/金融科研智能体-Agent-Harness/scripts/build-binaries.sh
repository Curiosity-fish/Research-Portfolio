#!/usr/bin/env bash
#
# Build tfa binaries for all platforms locally.
...
#     tfa-darwin-arm64.tar.gz
#     tfa-darwin-x64.tar.gz
#     tfa-linux-x64.tar.gz
#     tfa-linux-arm64.tar.gz
#     tfa-windows-x64.zip
#     tfa-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/tfa.exe"
    else
        bun build --compile --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/tfa"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating tfa-$platform.zip..."
        (cd "$platform" && zip -r ../tfa-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating tfa-$platform.tar.gz..."
        mv "$platform" tfa && tar -czf tfa-$platform.tar.gz tfa && mv tfa "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../tfa-$platform.zip)
    else
        tar -xzf tfa-$platform.tar.gz && mv tfa "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/tfa.exe"
    else
        echo "  $OUTPUT_DIR/$platform/tfa"
    fi
done
