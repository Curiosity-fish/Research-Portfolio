...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	const root = await mkdtemp(join(tmpdir(), "tfa-sync-versions-"));
	try {
		await writeManifest(root, "packages/ai", {
			name: "@earendil-works/tfa-ai",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/coding-agent", {
			name: "@earendil-works/tfa-coding-agent",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/evals", {
			name: "@earendil-works/tfa-evals",
			version: "9.9.9",
			private: true,
			dependencies: {
				"@earendil-works/tfa-coding-agent": "^1.0.0",
				"@mariozechner/tfa-ai": "npm:@earendil-works/tfa-ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@earendil-works/tfa-coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/tfa-ai"], "npm:@earendil-works/tfa-ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@earendil-works/tfa-coding-agent"], "^1.0.0");
