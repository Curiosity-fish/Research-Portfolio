...
TFA and also where the boundaries are.

In general TFA is a coding agent that runs locally within the security boundary
...
TFA treats the local user account and files writable by that account as inside
the same trust boundary as the TFA process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
TFA configuration, they can generally influence TFA or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how TFA grants that write access or crosses
an operating-system privilege boundary.

TFA relies on users installing trustworthy extensions and loading trustworthy
skills and only to use tfa within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in tfa or another package in
...
- Local code execution or sandboxing behavior (the TFA coding agent intentionally does not have a sandbox)
- Behavior of tfa extensions or skills installed by the user
...
- Public internet exposure of a TFA installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.tfa`,
  `~/.tfa/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how TFA itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the tfa coding agent.
...
For example, a report showing that malicious contents written to a trusted TFA
configuration file cause TFA to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through TFA.  For exposed-secret reports,
