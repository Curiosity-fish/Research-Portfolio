...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@earendil-works/tfa-coding-agent";
import { Container, hyperlink, Text } from "@earendil-works/tfa-tui";
...
async function fetchAdvisoryMetadata(tfa: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await tfa.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(tfa, cwd, target);
	}
	...
	try {
		const result = await tfa.exec("gh", args);
...
export default function promptUrlWidgetExtension(tfa: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = tfa.getSessionName()?.trim();
		if (!currentName) {
			tfa.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(tfa, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	tfa.on("session_switch", async (_event, ctx) => {
