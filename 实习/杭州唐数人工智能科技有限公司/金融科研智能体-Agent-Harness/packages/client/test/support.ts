...
import type { ByteTransport, ByteTransportHandlers, TfaSessionHandle } from "../src/index.ts";
...
export function createClient(server: MemoryByteServer): TfaClient {
	return new TfaClient({
		transportFactory: (handlers) => server.connect(handlers),
	});
}

export async function connectClient(server: MemoryByteServer): Promise<TfaClient> {
