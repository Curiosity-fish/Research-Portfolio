...
import { TfaClient } from "../src/index.ts";
...
test("rejects invalid Unix transport options", () => {
	...
	expect(() => createUnixTransportFactory({ path: "/tmp/tfa.sock", maxPendingBytes: 0 })).toThrow(/positive/);
});

describe.runIf(process.platform !== "win32")("Unix-domain sockets", () => {
	test("TfaClient exchanges fragmented framed messages over a real Unix socket", async () => {
		const directory = await mkdtemp(join(tmpdir(), "tfa-client-"));
		const socketPath = join(directory, "tfa.sock");
	...
	test("TfaClient rejects a truncated final frame from a real Unix socket", async () => {
		const directory = await mkdtemp(join(tmpdir(), "tfa-client-"));
		const socketPath = join(directory, "tfa.sock");
		const sockets = new Set<Socket>();
		const server = createServer((socket) => {
