import { PROTOCOL_VERSION } from "@earendil-works/tfa-protocol";
import { describe, expect, test } from "vitest";
import { TfaClient, TfaClientDisposedError } from "../src/index.ts";
import { attachSession, baseServerSnapshot, connectClient, MemoryByteServer, sessionSnapshot } from "./support.ts";

describe("TfaClient disposal", () => {
	test("connects through its ownership factory", async () => {
		...
		const client = await TfaClient.connect({
	...
	test("disconnects, invalidates child handles, and rejects pending requests", async () => {
		...
		await expect(pending).rejects.toBeInstanceOf(TfaClientDisposedError);
		await expect(handle.prompt("after disposal")).rejects.toBeInstanceOf(TfaClientDisposedError);
		await firstDisposal;
	});
