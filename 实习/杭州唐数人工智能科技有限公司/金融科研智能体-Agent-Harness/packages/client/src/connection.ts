...
import { TfaDisconnectedError, TfaServerError, toDisconnectedError, toError } from "./errors.ts";
...
export class Connection {
	...
	constructor(options: ConnectionOptions) {
		...
		if (
			...
			throw new TypeError(`TfaClient maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
		}
	}
	...
	connect(): Promise<ServerSnapshot> {
		if (this.#lifecycle.state !== "disconnected") {
			return Promise.reject(new TfaDisconnectedError(`TfaClient is already ${this.#lifecycle.state}`));
		}
	...
	disconnect(reason: string | Error = "Client disconnected"): void {
		...
		this.#failAndClose(typeof reason === "string" ? new TfaDisconnectedError(reason) : reason);
	}
	...
	send(frame: Uint8Array): void {
		...
		if (lifecycle.state !== "connected") throw new TfaDisconnectedError();
	...
	#handleMessage(message: ServerMessage): void {
		...
		if (lifecycle.state === "connecting") {
			if (message.type === "hello_error") {
				this.#failAndClose(new TfaServerError(message.error));
				return;
			}
	...
	#handleClose(): void {
		...
		let error: Error = new TfaDisconnectedError("Byte transport closed");
		try {
