...
export class TfaClient {
	readonly #options: TfaClientOptions;
	...
	constructor(options: TfaClientOptions) {
	...
	static async connect(options: TfaClientOptions): Promise<TfaClient> {
		const client = new TfaClient(options);
		try {
	...
	connect(): Promise<ServerSnapshot> {
		if (this.#disposed) return Promise.reject(new TfaClientDisposedError());
	...
	async createSession(options: CreateSessionOptions = {}): Promise<TfaSessionHandle> {
	...
	async attachSession(sessionId: string): Promise<TfaSessionHandle> {
		return this.acquireSession(sessionId, { mode: "shared" });
	}

	async acquireSession(sessionId: string, options: AcquireSessionOptions): Promise<TfaSessionHandle> {
		this.#assertNotDisposed();
	...
	#request<const TCommand extends Command>(command: TCommand): Promise<ResultForCommand<TCommand>> {
		if (this.#disposed) return Promise.reject(new TfaClientDisposedError());
		if (!this.connected) return Promise.reject(new TfaDisconnectedError());
	...
	#createSessionLease(sessionId: string, token: SessionLeaseToken): TfaSessionHandle {
		...
		const assertActive = () => {
			this.#assertNotDisposed();
			if (!this.connected) throw new TfaDisconnectedError();
			if (!isActive()) throw new TfaSessionDetachedError(sessionId);
		};
	...
	#handleMessage(message: ResponseEnvelope | EventEnvelope): void {
		...
		if (!message.ok) {
			pending.reject(new TfaServerError(message.error));
			return;
		}
	...
	#handleConnectionStateChange(change: ConnectionStateChange): void {
		if (change.state === "disconnected") {
			...
			this.#rejectPendingRequests(change.error ?? new TfaDisconnectedError());
		}
	...
	dispose(): Promise<void> {
		...
		const error = new TfaClientDisposedError();
	...
	#assertNotDisposed(): void {
		if (this.#disposed) throw new TfaClientDisposedError();
	}
	...
	#reserveSessionLease(sessionId: string, mode: SessionLeaseMode): SessionLeaseToken {
		...
		if (mode === "exclusive" && count > 0) {
			throw new TfaSessionOwnershipError(sessionId, `Session ${sessionId} already has an active lease`);
		}
		if (mode === "shared" && this.#exclusiveSessionLeases.has(sessionId)) {
			throw new TfaSessionOwnershipError(sessionId, `Session ${sessionId} has an exclusive lease`);
		}
