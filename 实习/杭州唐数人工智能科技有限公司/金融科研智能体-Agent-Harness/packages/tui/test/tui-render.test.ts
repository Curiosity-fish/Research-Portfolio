...
describe("TUI debug logging", () => {
	it("writes redraw logs to the provided directory", async () => {
		const logDir = mkdtempSync(join(tmpdir(), "tfa-tui-log-"));
		try {
			await withEnv({ TFA_DEBUG_REDRAW: "1" }, async () => {
				const terminal = new VirtualTerminal(40, 10);
				...
				assert.match(readFileSync(join(logDir, "tfa-debug.log"), "utf-8"), /fullRender: first render/);
				tui.stop();
			});
		} finally {
