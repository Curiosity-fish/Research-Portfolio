import type { TelemetryContext } from "@earendil-works/tfa-telemetry";
...
import type { TfaMessagesOptions } from "./api/tfa-messages.ts";
...
export interface StreamOptions extends ProviderRequestOptions<Model<Api>> {
	 ...
	 * SGLang, ...) receive parameters tfa does not model, e.g. `top_p`, `top_k`, `min_p`,
...
export interface ApiOptionsMap {
	...
	"tfa-messages": TfaMessagesOptions;
}
...
export interface OpenAICompletionsCompat {
	...
	/** Whether streamed responses include `finish_reason`. When false, tfa infers `stop` or `toolUse` when the stream ends. Default: true. */
	...
	thinkingFormat?:
		...
		| "string-thinking"
		| "ant-ling";
	/** Kwargs to send as `chat_template_kwargs` when `thinkingFormat` is `chat-template`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for tfa-controlled thinking values. */
	chatTemplateKwargs?: Record<string, ChatTemplateKwargValue>;
	/** Arguments to send as `chat_template_args` when `thinkingFormat` is `baseten`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for tfa-controlled thinking values. */
...
export interface Model<TApi extends Api> {
	...
	reasoning: boolean;
	/**
	 * Maps tfa thinking levels to provider/model-specific values.
