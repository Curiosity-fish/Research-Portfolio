...
export const stream: StreamFunction<"bedrock-converse-stream", BedrockOptions> = (
	...
	(async () => {
		...
		// A profile explicitly configured through tfa's auth flow (the `profile`
...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "tfa-ai-custom-headers", priority: "low" });
}
 ...
 * Defaults to "short" and uses TFA_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("TFA_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}
