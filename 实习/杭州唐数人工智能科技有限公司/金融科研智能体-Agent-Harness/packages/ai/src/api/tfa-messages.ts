/**
 * tfa-messages API implementation.
 *
 * Streams tfa's own message protocol directly to a backend: the request is a
 ...
 * models.json custom provider with `"api": "tfa-messages"`.
 */
...
export interface TfaMessagesOptions extends StreamOptions {
...
type TfaMessagesUsage = AssistantMessage["usage"];
type TfaMessagesStopReason = AssistantMessage["stopReason"];

/** Impact summary of a server-side message rewrite (e.g. a gateway policy). */
export type TfaMessagesRewriteImpact = {
	policyId: string;
...
/** Serialized assistant-message event as sent by a tfa-messages backend. */
export type TfaMessagesEvent =
	| { type: "start" }
	...
	| {
			type: "done";
			reason: Extract<TfaMessagesStopReason, "stop" | "length" | "toolUse">;
			usage: TfaMessagesUsage;
			responseId?: string;
			rewrite?: TfaMessagesRewriteImpact;
	  }
	| {
			type: "error";
			reason: Extract<TfaMessagesStopReason, "aborted" | "error">;
			usage: TfaMessagesUsage;
			errorMessage?: string;
			responseId?: string;
			rewrite?: TfaMessagesRewriteImpact;
...
export class TfaMessagesResponseError extends Error {
	code?: string;
	...
	constructor(message: string, code: string | undefined, diagnosticDetails: Record<string, unknown>) {
		super(message);
		this.name = "TfaMessagesResponseError";
		this.code = code;
		this.diagnosticDetails = diagnosticDetails;
	}
}

function parsePiMessagesErrorBody(body: string): TfaMessagesErrorBody | undefined {
	try {
		const parsed = JSON.parse(body) as TfaMessagesErrorBody | null;
...
function formatPiMessagesResponseError(
	response: Response,
	body: string,
	errorBody: TfaMessagesErrorBody | undefined,
): string {
...
function createPiMessagesResponseError(
	model: Model<"tfa-messages">,
	url: URL,
	response: Response,
	body: string,
): TfaMessagesResponseError {
	const errorBody = parsePiMessagesErrorBody(body);
	const code = typeof errorBody?.error?.code === "string" ? errorBody.error.code : undefined;
	return new TfaMessagesResponseError(formatPiMessagesResponseError(response, body, errorBody), code, {
		version: 1,
...
function createEmptyUsage(): TfaMessagesUsage {
	return {
		input: 0,
		output: 0,
		cacheRead: 0,
...
function appendRewriteDiagnostic(message: AssistantMessage, rewrite: TfaMessagesRewriteImpact | undefined): void {
	if (!rewrite) {
		return;
	}
	appendAssistantMessageDiagnostic(message, {
		type: "tfa_messages_rewrite",
		timestamp: Date.now(),
		details: { ...rewrite },
	});
}

function createEventConverter(model: Model<"tfa-messages">) {
	...
	return (event: TfaMessagesEvent): AssistantMessageEvent => {
		switch (event.type) {
			case "done":
...
async function* readPiMessagesEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<TfaMessagesEvent> {
...
function parsePiMessagesEvent(raw: string): TfaMessagesEvent | undefined {
	const data = raw
		.split("\n")
	...
	return data && data !== "[DONE]" ? (JSON.parse(data) as TfaMessagesEvent) : undefined;
}

function createErrorEvent(model: Model<"tfa-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof TfaMessagesResponseError) {
		appendAssistantMessageDiagnostic(
			assistantMessage,
			createAssistantMessageDiagnostic("tfa_messages_response_failure", error, error.diagnosticDetails),
		);
	}
...
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention | undefined {
	...
	return getProviderEnvValue("TFA_CACHE_RETENTION", env) === "long" ? "long" : undefined;
}

export const stream: StreamFunction<"tfa-messages", TfaMessagesOptions> = (
	model: Model<"tfa-messages">,
	context: Context,
	options?: TfaMessagesOptions,
	...
	void (async () => {
		try {
			...
			for await (const tfaEvent of readPiMessagesEvents(response.body)) {
				const event = convertEvent(tfaEvent);
...
export const streamSimple: StreamFunction<"tfa-messages", SimpleStreamOptions> = (
	model: Model<"tfa-messages">,
	context: Context,
	options?: SimpleStreamOptions,
): AssistantMessageEventStream => {
	const extra = options as TfaMessagesOptions | undefined;
