...
import { stream, streamSimple, type TfaMessagesOptions } from "../src/api/tfa-messages.ts";
...
function createModel(baseUrl: string): Model<"tfa-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
		api: "tfa-messages",
...
describe("tfa-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		const { baseUrl, requests } = await startServer({
			headers: { "x-tfa-gateway-upstream-provider": "anthropic" },
			events: [{ type: "done", reason: "stop", usage }],
		});
		const model = createModel(baseUrl);

		let observedHeaders: Record<string, string> | undefined;
		const options: TfaMessagesOptions = {
			apiKey: "test-key",
			debug: true,
			onResponse: (response) => {
		...
		expect(observedHeaders?.["x-tfa-gateway-upstream-provider"]).toBe("anthropic");
	});
	...
	it("surfaces backend error responses with diagnostics", async () => {
		...
		expect(message.diagnostics?.[0]?.type).toBe("tfa_messages_response_failure");
...
describe("tfa-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("tfa-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "tfa-messages";
		expect(api).toBe("tfa-messages");
	});
});
