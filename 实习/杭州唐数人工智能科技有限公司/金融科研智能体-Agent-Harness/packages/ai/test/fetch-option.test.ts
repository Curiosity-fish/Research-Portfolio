...
import { streamSimple as streamPiMessages } from "../src/api/tfa-messages.ts";
...
describe("fetch stream option", () => {
	...
	it("uses fetch for Mistral, Codex SSE, and tfa-messages HTTP requests", async () => {
		const { custom, fallback } = mockFetches();
		...
		await streamPiMessages(createModel("tfa-messages"), context, {
			apiKey: "test-key",
			fetch: custom,
		}).result();
