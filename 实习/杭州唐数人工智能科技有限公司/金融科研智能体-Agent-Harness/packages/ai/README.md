...
```bash
npm install @earendil-works/tfa-ai
```

TypeBox exports are re-exported from `@earendil-works/tfa-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@earendil-works/tfa-ai';
import { builtinModels } from '@earendil-works/tfa-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@earendil-works/tfa-ai/providers/anthropic';
import { openaiProvider } from '@earendil-works/tfa-ai/providers/openai';
import { openrouterProvider } from '@earendil-works/tfa-ai/providers/openrouter';
import { amazonBedrockProvider } from '@earendil-works/tfa-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@earendil-works/tfa-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@earendil-works/tfa-ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. tfa-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@earendil-works/tfa-ai';
...
API-key credentials use the same discriminator as tfa's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@earendil-works/tfa-ai';
...
```typescript
import { validateToolCall, type Tool } from '@earendil-works/tfa-ai';
...
```typescript
import { builtinImagesModels } from '@earendil-works/tfa-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@earendil-works/tfa-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@earendil-works/tfa-ai';
import { openAICompletionsApi } from '@earendil-works/tfa-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@earendil-works/tfa-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@earendil-works/tfa-ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are tfa thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@earendil-works/tfa-ai/api/anthropic-messages';
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for tfa-controlled thinking values
  chatTemplateArgs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_args values for thinkingFormat: 'baseten'; use $var for tfa-controlled thinking values
...
```typescript
import { createModels, type Context } from '@earendil-works/tfa-ai';
import { anthropicProvider } from '@earendil-works/tfa-ai/providers/anthropic';
import { openaiProvider } from '@earendil-works/tfa-ai/providers/openai';
import { googleProvider } from '@earendil-works/tfa-ai/providers/google';
...
```typescript
import { createModels } from '@earendil-works/tfa-ai';
import { anthropicProvider } from '@earendil-works/tfa-ai/providers/anthropic';
...
- `@earendil-works/tfa-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@earendil-works/tfa-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@earendil-works/tfa-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@earendil-works/tfa-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@earendil-works/tfa-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@earendil-works/tfa-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@earendil-works/tfa-ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `TFA_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @earendil-works/tfa-ai login              # interactive provider selection
npx @earendil-works/tfa-ai login anthropic    # login to specific provider
npx @earendil-works/tfa-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@earendil-works/tfa-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@earendil-works/tfa-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@earendil-works/tfa-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@earendil-works/tfa-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@earendil-works/tfa-ai`.
