...
- **Lanes.** A session hosts one or more lanes. A lane is a named position in the conversation tree. Each lane runs at most one operation at a time. Lanes run in parallel. A run and its queued messages belong to the lane that accepted them. Example: a Slack channel is a session; each thread is a lane. Interactive tfa uses one lane and does not show the concept in its UI. Extensions get the full harness API, including lanes. Example: a subagent tool runs on a second lane of its parent's session.
...
- **Provider stream resumption.** Partial streams are never persisted. An interrupted streaming request is retried or abandoned. Deferred requests are different and in scope: the provider returns a handle at once and serves the result later (e.g. `background: true` on a Responses API, batch APIs). tfa-ai returns an assistant message with stop reason `deferred` that carries the handle; it is persisted like any assistant message. Redeeming the handle appends a normal assistant message. Recovery sees the unredeemed handle and fetches instead of paying for a new request.
...
`desiredMaxOutput` is the caller-supplied `maxTokens` when set, else `model.maxTokens` — the intended limit **before** any context clamping. The value actually sent can never be the reference: some providers reject an explicit output cap outright (the OpenAI Codex backend returns HTTP 400 for `max_output_tokens`), and TFA clamps others to the remaining context. This covers a context-clamped request that returns 16 reasoning tokens against a 128k intent (recover), a Xiaomi/Qwen-style `length` with zero output (recover), and an explicit 1,024 cap fully used (genuine stop) — with no context-percentage heuristics. Overflow-form errors — a provider rejection matching the overflow patterns, or a silent success whose prompt exceeds the window — classify the same way and take the same path.
...
This is the target H0 surface. At the I0 landing, `AgentHarnessOptions` still exposes the scaffold-only `context?: ExecutionContext`; it does not yet accept `telemetryContext` or use the no-op context by default.
...
```ts
// Interactive tfa. suspended has 0 or 1 entries, always "main".
...
**Implementation status:** this section is the L1–L3 target. At the I0 landing, `agent-loop.ts` still exposes the legacy loop API over private monolithic helpers; it exports none of the `streamAssistant`, tool-phase, or batch building blocks below and threads no `TelemetryContext`.

L1–L3 split `agent-loop.ts` into building blocks. Blocks own no durable state and know nothing about sessions, records, or lanes. The harness will compose them and insert its durability writes between their phases. The existing `agentLoop` / `runAgentLoop` API stays in the file as a thin composition of the same blocks, with its current signature and behavior — existing consumers do not change.
...
The existing public interface of `agent-loop.ts` must not break. Every current export keeps its signature and behavior: `agentLoop`, `agentLoopContinue`, `runAgentLoop`, `runAgentLoopContinue`, `AgentEventSink`, and the config surface they consume (`getSteeringMessages`, `getFollowUpMessages`, `prepareNextTurn`, `shouldStopAfterTurn`, `beforeToolCall`, `afterToolCall`, event order included). L3 will reimplement them as thin compositions of `streamAssistant` and `executeToolBatch` with the no-op `TelemetryContext` — no durability, no new semantics. Acceptance criterion: the existing `agent-loop` and `agent` test suites pass unchanged.
...
## 16. tfa-ai: deferred requests

**Implementation status:** the tfa-ai option, handle, provider capability, authenticated `Models` dispatch, and faux-provider behavior in this section are landed. `TerminalStopReason` and `SettledAssistantMessage` are target agent-side narrowings for L1, not current tfa-ai exports. Harness redemption/cancellation behavior remains H8, and request-span instrumentation remains O2.

The provider-level interface the future harness builds on is already available. Everything is per-request; batch APIs can implement the same shape through a custom provider.
...
// Target agent-side narrowings added by L1; tfa-ai currently exports StopReason
// and AssistantMessage, but not these two aliases.
...
interface ProviderRequestOptions<TModel = Model<Api>> {
  signal?: AbortSignal;
  /** Explicit parent for this logical tfa-ai operation. Inherited by stream,
...
`ProviderRequestOptions.telemetryContext` is inherited by `StreamOptions`, `SimpleStreamOptions`, `DeferredFetchOptions`, `DeferredCancelOptions`, and `ImagesOptions`; provider, `Models`, `ImagesModels`, and direct stream/image dispatch preserve it unchanged. `buildBaseOptions()` also preserves it when built-in `streamSimple()` implementations convert to provider-specific stream options. These are landed I0 behaviors.

`pending` is internal to a mutable live-stream message. L1 will narrow request-wrapper results to the target `SettledAssistantMessage`; H8 will enforce that harness-written entries and durable usage records cannot contain `pending`; O2 will apply the same rule to settled `tfa.ai.request` spans and normalize terminal `toolUse` to `tool_use`.

The future harness runtime will not talk to a provider object directly; it will use the already-landed authenticated dispatch surface:
...
H8 will treat a terminal fetch answer as final for the run: the harness will append the error message and fail the operation (section 6), never start an automatic replacement request, and convert a rejected fetch promise into the same `stopReason: "error"` message form so expected provider and authentication failures stay in-band. On a returned still-deferred message it will require the complete handle to equal the persisted handle: a provider cannot replace durable handle data without a write, so a mismatch is a defect.

Deferred assistant messages carry a handle, not content. The landed session context projection already omits them from provider context; H8 will wire durable suspension and redemption around that projection.
...
- The fork point may be any message entry. A copy whose tip sits mid-tool-batch is still promptable: tfa-ai's transformMessages inserts synthetic empty results for orphaned tool calls at request build time.
...
Telemetry uses explicit context propagation. Core code does not use `AsyncLocalStorage`, global current-span state, or runtime-specific context APIs: tfa runs in Node, Bun, browsers, and workers, so no runtime's ambient-context mechanism can be the core abstraction. An adapter may use ambient context internally — for example, an OpenTelemetry adapter may activate its native child context so HTTP auto-instrumentation attaches correctly — but tfa always passes the parent explicitly.

TFA ships no exporter and requires no backend-specific telemetry implementation. The application supplies a `TelemetryContext`; an **adapter** implements that interface and bridges spans into OTel, Sentry, logs, or another backend. The adapter is trusted to obey the callback contract below. It owns backend ids and native context objects; core never carries trace-id plumbing.

### Package ownership

The generic contract, schema-definition machinery, and shared no-op implementation live in `packages/telemetry/src/index.ts` and are exported from `@earendil-works/tfa-telemetry`. TFA-ai imports only `TelemetryContext` for request options; it owns no span schema or helper and emits no telemetry itself. `packages/agent/src/harness/telemetry.ts` owns both `AI_TELEMETRY_SCHEMA` / `startAiSpan()` and `HARNESS_TELEMETRY_SCHEMA` / `startHarnessSpan()`. The agent package root re-exports those domain schemas and helpers plus the generic telemetry surface. There is one generic contract and one current domain-schema owner.

The landed `ProviderRequestOptions.telemetryContext` carries an optional parent through stream/simple-stream, deferred fetch/cancel, and image request options; provider, `Models`, `ImagesModels`, direct dispatch, and built-in simple-option conversion preserve the object unchanged. This leaves tfa-ai API implementations free to use the context later without making tfa-ai own the generic API today. The current `AgentHarnessOptions` still has its temporary `context?: ExecutionContext` field. H0 will replace that scaffold surface with `telemetryContext?: TelemetryContext`, L/I/H work will thread it, and O2 will make the agent-side request wrapper emit `tfa.ai.request` through the agent-owned AI schema.

Both schemas are tfa-owned. Span names use the `tfa.ai.*`, `tfa.harness.*`, and `tfa.session.*` families; attributes use the same tfa-owned `tfa.*` vocabulary and do not adopt an external semantic-convention namespace. Adapters translate them when useful; the emitted tfa vocabulary remains stable regardless of backend convention churn.
...
Adapters preserve the callback's result and error. Their recording methods are synchronous, passive, and must not throw; asynchronous exporters buffer internally and flush on their own schedule. If native span creation or recording fails, the adapter suppresses that failure, substitutes no-op behavior, and still invokes the business callback exactly once. A nonconforming adapter is an application defect. The no-op implementation invokes the callback with one shared inert span, allocates no per-span object, inspects and retains no attributes, and otherwise preserves the callback's behavior. Flushing a real adapter at shutdown is the application's responsibility.

The target L/I/H runtime passes context to every effectful implementation boundary as a normal argument. No core function will look up a current context:
...
A `TelemetrySpan` is also the explicit child `TelemetryContext`. Passing the callback span to lower-level work creates nesting through the ordinary call graph. I4 will make every `Effects` method receive its parent as a parameter; O2 will give parallel tools separate child spans and therefore separate parent contexts.
  ...
  spans: {
    "tfa.ai.request": {
      ...
      endAttributes: {
        ...
        "tfa.ai.response.id": {
          type: "string", cardinality: "high", description: "Provider response id",
        },
        ...
        "tfa.ai.usage.cache_write_tokens": { type: "number", description: "Reported cache-write tokens" },
        ...
        "tfa.ai.stream.time_to_first_chunk_ms": {
          type: "number", description: "Elapsed milliseconds to first update chunk",
        },
        "tfa.ai.error.type": {
          type: "string", cardinality: "low", description: "Provider or transport error class",
        },
      },
...
type AiSpanStartAttributes<Name extends AiSpanName> = InferStartAttributes<
  (typeof AI_TELEMETRY_SCHEMA.spans)[Name]["startAttributes"]
>;
type AiSpanEndAttributes<Name extends AiSpanName> = InferOptionalAttributes<
  (typeof AI_TELEMETRY_SCHEMA.spans)[Name]["endAttributes"]
>;
type AiSpanAttributes<Name extends AiSpanName> = AiSpanStartAttributes<Name>
  & AiSpanEndAttributes<Name>;
```
...
`AI_TELEMETRY_SCHEMA` declares no tfa-written span events and one span. Its parent rule is `{ kind: "any" }`:

| span | allowed parents | status |
|---|---|---|
| `tfa.ai.request` | root or any caller span | error on throw/reject or a returned result with stop reason `error`; `aborted` and `deferred` are normal outcomes |

| `tfa.ai.request` start attribute | type | requirement | values / meaning |
|---|---|---|---|
| `tfa.ai.operation` | string | ! | `stream`, `fetch_deferred`, `cancel_deferred`, `generate_images` |
| `tfa.ai.provider` | string | ! | selected provider id |
| `tfa.ai.model` | string | ! | requested model id |
| `tfa.ai.api` | string | ! | provider API id |
| `tfa.ai.streaming` | boolean | ! | whether this operation returns a stream |
| `tfa.ai.deferred` | boolean | ? | whether the operation requests or participates in deferred execution |

| `tfa.ai.request` end attribute | type | values / meaning |
|---|---|---|
| `tfa.ai.response.model` | string | concrete response model, when reported |
| `tfa.ai.response.id` | string | provider response id; high cardinality |
| `tfa.ai.response.stop_reason` | string | `stop`, `length`, `tool_use`, `error`, `aborted`, `deferred`; terminal `toolUse` normalizes to `tool_use`, and `pending` is never recorded |
| `tfa.ai.http.status_code` | number | final HTTP status when exposed by the provider path |
...
| `tfa.ai.usage.reasoning_tokens` | number | reported reasoning subset of output |
| `tfa.ai.usage.total_tokens` | number | reported total tokens |
| `tfa.ai.usage.cost` | number | reported total cost |
| `tfa.ai.stream.chunk_count` | number | number of streamed update chunks, without chunk content |
...
The schema declares no per-chunk telemetry event. When O2 inserts runtime spans, the existing assistant stream will continue to carry live deltas while telemetry records only aggregate chunk count and first-chunk latency. Default O2 telemetry must never contain request or response content.

#### Harness schema

The three operation spans share `tfa.session.id` (string, required, high cardinality), `tfa.lane.name` (string, required, high cardinality), `tfa.operation.id` (string, required, high cardinality), and `tfa.operation.recovery` (boolean, required). Each also requires `tfa.operation.kind` with only the literal matching that span. Operation error status may add optional end attributes `tfa.error.code` and `tfa.error.type`, both low-cardinality strings; free-form error messages are status diagnostics, not schema attributes.

| span | allowed parents | start attributes | optional end attributes | explicit error status |
|---|---|---|---|---|
| `tfa.harness.run` | root or application span | common operation attributes plus `tfa.operation.kind`: `run` | `tfa.operation.outcome`: `completed`, `aborted`, `failed`, `suspended` | outcome `failed` |
| `tfa.harness.compaction` | root or application span | common operation attributes plus `tfa.operation.kind`: `compaction` | `tfa.operation.outcome`: `completed`, `declined`, `aborted`, `failed` | outcome `failed` |
| `tfa.harness.navigation` | root or application span | common operation attributes plus `tfa.operation.kind`: `navigation` | `tfa.operation.outcome`: `completed`, `declined`, `aborted`, `failed` | outcome `failed` |
| `tfa.harness.checkpoint` | `tfa.harness.run` | `tfa.lane.name`!, `tfa.operation.id`!, `tfa.checkpoint.kind`!: `normal`, `failure_drain`, `abort_reconcile` | none | only throw/reject |
| `tfa.harness.turn` | `tfa.harness.run` | `tfa.lane.name`!, `tfa.operation.id`!, `tfa.turn.id`! string, high cardinality | none | only throw/reject |
| `tfa.harness.step` | `tfa.harness.turn`, `tfa.harness.checkpoint`, `tfa.harness.compaction`, or `tfa.harness.navigation` | `tfa.lane.name`!, `tfa.operation.id`!, `tfa.step.kind`!: `assistant`, `compaction`, `branch_summary`; `tfa.step.attempt`! number; `tfa.compaction.reason`?: `manual`, `threshold`, `overflow` | `tfa.step.outcome`: `succeeded`, `retry`, `failed`, `aborted`, `deferred`, `overflow` | outcome `retry` or `failed` |
| `tfa.harness.tool` | `tfa.harness.turn` for live work or `tfa.harness.run` for reconciliation | `tfa.lane.name`!, `tfa.operation.id`!, `tfa.turn.id`? string high-cardinality, `tfa.tool.name`! string, `tfa.tool.call_id`! string high-cardinality, `tfa.tool.replay`!: `never`, `safe`; `tfa.tool.recovery`! boolean | `tfa.tool.is_error` boolean for the raw phase-2 execution result | `tfa.tool.is_error: true` |
| `tfa.harness.hook` | root or the current harness/AI scope | `tfa.lane.name`!, `tfa.operation.id`? string high-cardinality, `tfa.hook.name`! string with values from `HookName`, `tfa.hook.registration_id`? string | `tfa.hook.outcome`: `completed`, `skipped`, `blocked`, `failed` | handler throw, including fail-closed `before_tool` |
| `tfa.harness.sleep` | `tfa.harness.step` or `tfa.harness.run` | `tfa.operation.id`!, `tfa.sleep.delay_ms`! number | `tfa.sleep.outcome`: `elapsed`, `aborted` | only throw/reject |
| `tfa.harness.event_handler` | root or the scope emitting the event | `tfa.event.type`! low-cardinality string with the section 10 event discriminants, `tfa.lane.name`? string high-cardinality | none | listener throw; the event system catches it after the span rejects |
| `tfa.session.write` | root or the current harness scope | `tfa.lane.name`!, `tfa.operation.id`? string high-cardinality, `tfa.session.mutation`!: `entry`, `record`, `lane`, `fact`; `tfa.session.item_type`? string | `tfa.session.seq` number when the committed API exposes it | storage rejection |

The parent column maps directly to `TelemetryParentDefinition`: “root or application span” is `root_or_external`; “root or the current scope” and “root or any caller span” are `any`; every finite tfa span list uses `spans` with exactly those names. Under O2, `tfa.harness.tool` will wrap phase 2 (`executeTool`) only and settle before `after_tool` finalization: `tfa.tool.is_error` describes the raw execution result, there is no final `terminate` attribute, and blocked or invalid calls that never execute emit no tool span. Live execution will supply the active turn id and parent the span to `tfa.harness.turn`; reconciliation has no durable turn id, omits it, and parents the span directly to the resumed `tfa.harness.run` invocation. The landed `tfa.hook.name` values array is exactly `before_run`, `before_resume`, `before_run_end`, `transform_context`, `before_request`, `before_payload`, `after_response`, `before_tool`, `after_tool`, `before_compaction`, and `before_navigation`. The landed `tfa.event.type` values array contains every `type` discriminant in the section 10 catalog and no others. `tfa.harness.hook` describes one registered handler invocation, so O2 can give isolated handler failures their own status without failing the enclosing run. `tfa.harness.event_handler` does the same for passive listener failures. The harness schema declares no span events initially.

Dynamic identifiers and names are attributes, never span names. The landed schema definitions are the exhaustive vocabulary O2 may emit; I0 itself emits no tfa-written runtime span, event, or attribute.
...
**Implementation status:** this subsection is the O2 target, not landed I0 behavior.

Telemetry wrappers will follow ownership of ordinary work. The procedure layer will wrap orchestration scopes — operation invocation, checkpoint, turn, and retryable step — and pass each callback's `TelemetrySpan` as the parent parameter to work below it. `Effects` will wrap the atomic effect it owns. Telemetry is not part of the gated action vocabulary and creates no durable crash boundary.
...
async function assistantAttempt(
  ...
  return startHarnessSpan(
    ...
    "tfa.harness.step",
    {
      "tfa.lane.name": state.lane,
      "tfa.operation.id": op.id,
      "tfa.step.kind": "assistant",
      "tfa.step.attempt": attempt,
    },
    async (stepContext) => {
...
Under O2, section 14's `streamAssistant()` will be the logical model-request wrapper. It will start `tfa.ai.request` with `startAiSpan()`, pass that callback span as `ProviderRequestOptions.telemetryContext` through `Models`, record only schema-declared aggregate response fields, and return the same assistant message. `Effects.executeTool()` will similarly wrap only phase 2 in `tfa.harness.tool`; hook and event runners will follow the same explicit-parent pattern.

| owner / method | target telemetry |
|---|---|
| operation dispatcher | `tfa.harness.run`, `tfa.harness.compaction`, or `tfa.harness.navigation` |
| checkpoint / turn / step procedure scopes | corresponding `tfa.harness.*` scope span |
| `appendEntry`, `appendRecord`, `moveLane`, `setFact`, and a conditional commit that writes | `tfa.session.write`; a conditional no-write result emits no write span |
| `streamAssistant`, `fetchDeferred`, `cancelDeferred` | `tfa.ai.request` with the matching `tfa.ai.operation` |
| `executeTool` | `tfa.harness.tool` |
| `runHook` | one `tfa.harness.hook` per registered handler |
| `sleep` | `tfa.harness.sleep` |
| passive event delivery | one `tfa.harness.event_handler` per listener |
...
The target span tree follows execution scopes:

```text
tfa.harness.run
├─ tfa.harness.checkpoint
│  └─ tfa.harness.step          compaction, attempt
├─ tfa.harness.turn
│  ├─ tfa.harness.step          assistant, attempt
│  │  ├─ tfa.ai.request         provider, model, stop reason
│  │  └─ tfa.harness.sleep      retry delay
│  └─ tfa.harness.tool          tool name, call id, replay
├─ tfa.harness.hook
├─ tfa.harness.event_handler
└─ tfa.session.write            entry/record/lane/fact

tfa.harness.compaction          manual operation
tfa.harness.navigation
```

The procedure layer owns operation, checkpoint, turn, and step scopes. `Effects` owns session writes, phase-2 tool execution, hooks, and sleep. The request-dispatch wrapper around `Models` owns `tfa.ai.request`; passive event delivery owns handler spans. Each owner receives its parent context explicitly.

### Safety and testing

O2 default attributes will carry only schema-declared identifiers, names, counts, durations, stop reasons, status codes, and usage. They must never carry prompts, completions, tool arguments, tool output, file content, provider payloads, headers, or credentials. Schema fields flag any future sensitive or high-cardinality attribute explicitly.

Current I0 tests cover schema inference and exactness, JSON serialization, generated-document freshness, no-op synchronous admission and result/error preservation, shared inert-span behavior, payload non-inspection, request-option propagation, and closed-set rejection. TFA ships no recording adapter, so the merge, status, and post-settlement rules above are adapter conformance obligations rather than stateful production behavior exercised by I0. O2's runtime tests will use a capture adapter to assert that each emitted start/end/event bag conforms to the owning schema after every status path; no test requires an end attribute to exist. Content and secret fixtures will assert absence, not merely redaction.
...
- The tfa-ai deferred request types, `Models.fetchDeferred()` / `cancelDeferred()` dispatch, and faux-provider pending/ready/terminal/cancellation support are already implemented and tested. Only harness integration remains.
...
- [x] **I0 — telemetry contracts, typed schemas, and no-op context.** Dependencies: none.
  ...
  - In tfa-ai, add optional `telemetryContext` to `ProviderRequestOptions` so every stream, deferred, and image option inherits it; provider, `Models`, `ImagesModels`, direct dispatch, and simple-option conversion preserve it. TFA-ai owns no domain schema or helper.
  ...
  - Generate the combined repository-only Markdown reference from the runtime schema values with the named agent package scripts. Production helpers perform no runtime schema validation; schemas compile-time-check each tfa-written start/end/event call and remain importable as machine-readable data.
  - Wire telemetry before tfa-ai in workspace, local-release, publish, profiling, and coding-agent binary build order; add source-test aliases and refresh workspace/generated dependency locks.
  - Landed coverage: focused tests exercise no-op synchronous admission, returned-value and sync/async rejection preservation, explicit no-op child propagation, one shared frozen inert span with no payload inspection, exact start/optional-end inference, rejection of missing, unknown, empty-schema, and invalid closed-set attributes, absence of declared span events, schema JSON serialization, option propagation across provider/`Models` stream and deferred dispatch, direct and `ImagesModels` image dispatch, built-in simple-option conversion, and generated-document freshness. TFA ships no recording adapter; section 18's merge/status/post-settlement rules are adapter obligations, while O2 will test tfa's runtime status and nesting behavior with captured spans.
...
- [ ] **H8 — deferred provider redemption.** Dependencies: H7.
  - Integrate the already-landed tfa-ai deferred APIs: suspend, pending re-park, ready continuation, terminal/rejected fetch failure, handle mismatch, and best-effort cancellation.
...
13. `packages/agent/src/harness/compaction/compaction.ts` — preparation and split-turn summaries.
14. `packages/ai/src/utils/transform-messages.ts` — orphaned-tool-call healing.
15. `packages/coding-agent/src/core/agent-session.ts` — read-only behavioral reference; do not modify it.
16. `packages/coding-agent/src/core/extensions/runner.ts` — read-only error-isolation reference; do not modify it.
17. `packages/coding-agent/docs/session-format.md` — read-only v3 JSONL format reference.
