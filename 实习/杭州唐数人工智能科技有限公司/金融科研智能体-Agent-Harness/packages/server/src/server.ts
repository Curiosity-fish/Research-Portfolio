...
export class TfaServer {
	readonly id: string;

	private readonly listeners: readonly TfaServerListener[];
	...
	constructor(service: TfaServerService, options: TfaServerOptions) {
	...
	start(): Promise<this> {
		if (this.started) return Promise.reject(new Error("TfaServer is already started"));
		if (this.startPromise) return Promise.reject(new Error("TfaServer is already starting"));
		if (this.closing) return Promise.reject(new Error("TfaServer is closing or closed"));
		this.startPromise = this.startInternal();
		return this.startPromise;
	}

	private async startInternal(): Promise<this> {
		const started: TfaServerListener[] = [];
		try {
	...
	private toProtocolError(error: unknown): ProtocolError {
		...
		if (error instanceof TfaServerError) {
...
function resolveOptions(options: TfaServerOptions): { maxFrameLength: number; handshakeTimeoutMs: number } {
	if (!Array.isArray(options.listeners)) throw new TypeError("TfaServer listeners must be an array");
	if (options.serverId === "") throw new TypeError("TfaServer serverId must not be empty");
	const maxFrameLength = options.maxFrameLength ?? DEFAULT_MAX_FRAME_LENGTH;
	if (!Number.isSafeInteger(maxFrameLength) || maxFrameLength <= 0 || maxFrameLength > MAX_UINT32) {
		throw new TypeError(`TfaServer maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
	}
	...
	if (
		...
		throw new TypeError(`TfaServer handshakeTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
