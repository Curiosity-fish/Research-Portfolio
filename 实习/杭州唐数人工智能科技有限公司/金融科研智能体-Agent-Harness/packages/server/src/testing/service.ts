...
export class TestSessionRuntime implements TfaSessionRuntime {
	...
	async prompt(input: PromptInput): Promise<void> {
		if (this.getPhase() !== "idle") throw new TfaServerError("busy", "A prompt is already running");
	...
	async steer(input: PromptInput): Promise<void> {
		if (this.getPhase() === "idle") throw new TfaServerError("busy", "There is no active prompt to steer");
	...
	async abort(): Promise<void> {
		if (!this.pendingPrompt) throw new TfaServerError("busy", "There is no active prompt to abort");
		this.pendingPrompt.done.resolve("aborted");
	}

	async setModel(model: ModelRef): Promise<void> {
		if (this.getPhase() !== "idle") throw new TfaServerError("busy", "Session is busy");
		this.update({ model });
	}
	...
	emitError(error: TfaServerError): void {
...
export class TestServerService implements TfaServerService {
	...
	async createSession(options: CreateSessionOptions): Promise<TfaSessionRuntime> {
		this.lastCreatedId = options.id;
		if (this.sessions.has(options.id)) throw new TfaServerError("session_locked", "Session already exists");
		this.seed(options.id, options.name, options.cwd, options.model, options.thinkingLevel);
		return this.acquire(options.id);
	}

	async openSession(sessionId: string): Promise<TfaSessionRuntime> {
		if (!this.sessions.has(sessionId)) throw new TfaServerError("not_found", `Unknown session: ${sessionId}`);
		if (this.locked.has(sessionId)) throw new TfaServerError("session_locked", `Session is locked: ${sessionId}`);
	...
	seed(
		id = "session-1",
		name = `Session ${id}`,
		cwd = "/tmp/tfa-server-conformance",
