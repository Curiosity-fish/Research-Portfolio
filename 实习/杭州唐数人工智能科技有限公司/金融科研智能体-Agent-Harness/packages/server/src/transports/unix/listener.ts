...
import { DEFAULT_MAX_FRAME_LENGTH } from "@earendil-works/tfa-protocol";
...
class UnixListener implements TfaServerListener {
	...
	async start(accept: ByteConnectionAcceptor): Promise<void> {
		...
		validateUnixSocketPath(ownedBindPath, "TfaServer private Unix bind path");
...
export function createUnixListener(options: UnixListenerOptions): TfaServerListener {
...
function resolveUnixListenerOptions(options: UnixListenerOptions): ResolvedUnixListenerOptions {
	validateUnixSocketPath(options.path, "TfaServer Unix socket path");
	const mode = options.mode ?? DEFAULT_SOCKET_MODE;
	if (!Number.isInteger(mode) || mode < 0 || mode > 0o777) {
		throw new TypeError("TfaServer Unix socket mode must be an integer between 0 and 0o777");
	}
	...
	if (!Number.isSafeInteger(maxPendingBytes) || maxPendingBytes < maxFrameLength + 4) {
		throw new TypeError("TfaServer maxPendingBytes must be a safe integer at least maxFrameLength + 4");
	}
	...
	if (
		...
		throw new TypeError(`TfaServer gracefulCloseTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
	return {
		path: options.path,
		mode,
