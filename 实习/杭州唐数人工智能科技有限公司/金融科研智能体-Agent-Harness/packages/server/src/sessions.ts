...
import type { Command, EventEnvelope, SessionSnapshot, SessionSummary } from "@earendil-works/tfa-protocol";
import type { ByteConnection, ConnectionState } from "./connection.ts";
import { TfaServerError } from "./errors.ts";
import type { CreateSessionOptions, TfaServerService, TfaSessionRuntime, TfaSessionRuntimeEvent } from "./types.ts";

interface LiveSession {
	id: string;
	runtime: TfaSessionRuntime;
...
export class LiveSessionManager {
	...
	private async acquire(id: string, acquireRuntime: () => Promise<TfaSessionRuntime>): Promise<LiveSession> {
		for (;;) {
			const existing = this.liveSessions.get(id);
			if (existing) {
				if (existing.terminal) throw new TfaServerError("session_locked", `Session runtime is terminating: ${id}`);
	...
	private async create(id: string, acquireRuntime: () => Promise<TfaSessionRuntime>): Promise<LiveSession> {
		const runtime = await acquireRuntime();
		if (this.options.isClosing()) {
			await runtime.dispose();
			throw new Error("TfaServer closed while acquiring a session runtime");
		}
		let live: LiveSession | undefined;
		try {
			const snapshot = await runtime.snapshot();
			if (snapshot.id !== id) {
				throw new TfaServerError(
					"invalid_request",
	...
	private handleRuntimeEvent(live: LiveSession, event: TfaSessionRuntimeEvent): void {
		if (event.type === "error") {
	...
	private async terminate(live: LiveSession, error: TfaServerError): Promise<void> {
	...
	private async normalizedSnapshot(live: LiveSession): Promise<SessionSnapshot> {
		...
		if (snapshot.id !== live.id) {
			throw new TfaServerError("invalid_request", `Runtime session ID changed from ${live.id} to ${snapshot.id}`);
		}
		return {
			...snapshot,
	...
	private async attach(connection: ConnectionState, live: LiveSession): Promise<void> {
		if (connection.disconnected || connection.stage !== "ready" || connection.connection.closed) {
			await this.maybeDispose(live);
			throw new TfaServerError("invalid_request", "Connection closed while attaching to a session");
		}
	...
	private requireAttached(connection: ConnectionState, sessionId: string): LiveSession {
		if (!connection.sessionIds.has(sessionId)) {
			throw new TfaServerError("invalid_request", `Connection is not attached to session ${sessionId}`);
		}
		const live = this.liveSessions.get(sessionId);
		if (!live || live.terminal || live.disposing) {
			throw new TfaServerError("not_found", `Session is not live: ${sessionId}`);
		}
		return live;
	}
