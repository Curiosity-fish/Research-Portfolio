...
import { ServerMessageDecoder } from "@earendil-works/tfa-protocol";
import { afterEach, expect, test, vi } from "vitest";
import type { ByteConnection } from "../src/connection.ts";
import { TfaServer } from "../src/index.ts";
...
let server: TfaServer | undefined;
...
test("requires explicit listeners", () => {
	expect(() => Reflect.construct(TfaServer, [service, {}])).toThrow(/listeners/);
});
...
test("handshake timeout cleanup does not wait for a blocked output queue", async () => {
	...
	const core = new TfaServer(service, {
		listeners: [],
		maxFrameLength: 1024,
...
test("rejects timeout values above Node's maximum timer delay", () => {
	const unix = { path: "/tmp/tfa-server-timeout-test.sock" };
