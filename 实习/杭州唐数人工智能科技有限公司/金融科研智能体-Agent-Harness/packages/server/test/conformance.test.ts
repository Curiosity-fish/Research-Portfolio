...
import { encodeClientMessage, encodeFrame, PROTOCOL_VERSION } from "@earendil-works/tfa-protocol";
import { afterEach, describe, expect, test } from "vitest";
import { InternalServerError, NotImplementedError, type TfaServer, TfaServerError } from "../src/index.ts";
...
const servers = new Set<TfaServer>();
...
): Promise<{ server: TfaServer; service: TestServerService }> {
...
async function connect(server: TfaServer): Promise<ProtocolTestClient> {
...
describe("Unix transport conformance", () => {
	...
	test("disconnects attached clients when a runtime reports a terminal error", async () => {
		...
		runtime.emitError(new TfaServerError("session_locked", "lock ownership lost"));
		await client.waitForClose();
