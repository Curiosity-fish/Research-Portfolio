...
import type { ModelMetadata, SessionSnapshot, TranscriptProgress } from "@earendil-works/tfa-protocol";
import { afterEach, describe, expect, test } from "vitest";
import type { CreateSessionOptions, TfaServer, TfaSessionRuntime } from "../src/index.ts";
import {
...
async function connect(server: TfaServer): Promise<Client> {
...
describe("TfaServer Unix integration", () => {
	...
	test("rejects and disposes a service runtime with the wrong server-assigned ID", async () => {
		class WrongIdService extends MemoryService {
			override async createSession(options: CreateSessionOptions): Promise<TfaSessionRuntime> {
