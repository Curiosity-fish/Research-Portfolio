...
import { contentText } from "@earendil-works/tfa-ai";
import {
	type AgentSession,
...
import { TFA_SESSION_SNAPSHOT_ARTIFACT } from "./vitest-evals/artifacts.ts";

export type TfaCodingAgentInput = string | Array<{ type: "prompt"; content: string } | { type: "reload" }>;

type TfaCodingAgentModelSelection = {
	provider: string;
	id: string;
};

type TfaCodingAgentHarnessOptions = {
	name?: string;
	model?: TfaCodingAgentModelSelection;
	noTools?: CreateAgentSessionOptions["noTools"];
	transformSystemPrompt?: (defaultPrompt: string) => string;
};

type TfaCodingAgentHarnessWithOutput<TOutput extends JsonValue> = TfaCodingAgentHarnessOptions & {
	output: (args: { response: string; session: AgentSession }) => TOutput | Promise<TOutput>;
};

export function resolveModelSelection(
	explicitModel: TfaCodingAgentModelSelection | undefined,
	environment: { TFA_PROVIDER?: string; TFA_MODEL?: string } = process.env,
): TfaCodingAgentModelSelection {
	const provider = (explicitModel?.provider ?? environment.TFA_PROVIDER)?.trim();
	const id = (explicitModel?.id ?? environment.TFA_MODEL)?.trim();
	if (!provider || !id) {
		throw new Error("Select a harness model explicitly or set both TFA_PROVIDER and TFA_MODEL as defaults.");
	}
	return { provider, id };
}
...
async function runTfaCodingAgent<TOutput extends JsonValue>(
	input: TfaCodingAgentInput,
	signal: AbortSignal | undefined,
	setArtifact: HarnessContext["setArtifact"],
	options: TfaCodingAgentHarnessOptions | TfaCodingAgentHarnessWithOutput<TOutput>,
	...
	const root = await mkdtemp(join(tmpdir(), "tfa-eval-"));
	...
	try {
		...
		try {
			...
			if (response === undefined) throw new Error("TFA eval input must include at least one prompt step.");
	...
	if (sessionManager) {
		try {
			...
			if (sessionPath && existsSync(sessionPath)) {
				setArtifact(TFA_SESSION_SNAPSHOT_ARTIFACT, await readFile(sessionPath, "utf8"));
			}
		} catch (error) {
...
export function createTfaCodingAgentHarness<TOutput extends JsonValue>(
	options: TfaCodingAgentHarnessWithOutput<TOutput>,
): Harness<TfaCodingAgentInput, TOutput>;
export function createTfaCodingAgentHarness(
	options?: TfaCodingAgentHarnessOptions,
): Harness<TfaCodingAgentInput, string>;
export function createTfaCodingAgentHarness<TOutput extends JsonValue>(
	options: TfaCodingAgentHarnessOptions | TfaCodingAgentHarnessWithOutput<TOutput> = {},
) {
	return createHarness<TfaCodingAgentInput, string | TOutput>({
		name: options.name ?? "tfa-coding-agent",
		run: ({ input, signal, setArtifact }) => runTfaCodingAgent(input, signal, setArtifact, options),
	});
}
