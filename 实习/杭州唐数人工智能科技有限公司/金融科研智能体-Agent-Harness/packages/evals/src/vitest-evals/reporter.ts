...
import { persistEvalArtifactReferences, TFA_SESSION_SNAPSHOT_ARTIFACT } from "./artifacts.ts";
...
async function appendHarnessRunReport(test: TestCase): Promise<void> {
	const artifactDirectory = process.env.TFA_EVAL_ARTIFACT_DIR?.trim();
	...
	const metadata = Object.fromEntries(
		Object.entries(run.artifacts ?? {}).filter(
			([name]) => name !== "runId" && name !== TFA_SESSION_SNAPSHOT_ARTIFACT,
		),
	);
	const record = {
		schemaVersion: 1,
