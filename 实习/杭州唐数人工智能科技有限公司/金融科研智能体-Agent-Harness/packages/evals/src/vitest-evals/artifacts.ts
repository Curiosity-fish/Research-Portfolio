...
export const TFA_SESSION_SNAPSHOT_ARTIFACT = "tfaSessionJsonl";

const evalSessionArtifactKey = Symbol("tfa-evals-session-artifact");
const evalSourceArtifactKey = Symbol("tfa-evals-source-artifact");

interface TfaSessionAttachment extends TestAttachment {
	name: "session.jsonl";
...
interface TfaSessionArtifact extends TestArtifactBase {
	type: "@earendil-works/tfa-evals:session";
	runId: string;
	attachments: [TfaSessionAttachment] | [];
}

interface SourceArtifact extends TestArtifactBase {
	type: "@earendil-works/tfa-evals:source";
	runId: string;
...
declare module "vitest" {
	interface TestArtifactRegistry {
		[evalSessionArtifactKey]: TfaSessionArtifact;
...
export async function recordEvalSessionArtifact(
	...
	const session = run.artifacts?.[TFA_SESSION_SNAPSHOT_ARTIFACT];
	if (session === undefined) return;
	if (typeof runId !== "string" || typeof session !== "string") {
		throw new TypeError("TFA eval session artifact metadata is invalid.");
	}
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@earendil-works/tfa-evals:session" &&
				artifact.type !== "@earendil-works/tfa-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@earendil-works/tfa-evals:session" ? "sessions" : "sources";
