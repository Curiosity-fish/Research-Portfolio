...
TFA evals are behavioral, model-backed checks for TFA workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native TFA session artifacts.
...
```bash
TFA_PROVIDER=openai TFA_MODEL=gpt-5.6-sol npm run eval
```
...
Authentication comes from TFA's normal `ModelRuntime`, including TFA subscription credentials and provider API-key
...
native TFA session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. TFA-specific evals use `createTfaCodingAgentHarness(...)` from `src/tfa-harness.ts`, with one harness bound
...
const harness = createTfaCodingAgentHarness({ noTools: "all" });

describeEval("TFA smoke", { harness }, (it) => {
...
### Configuring the TFA harness

`createTfaCodingAgentHarness(...)` accepts:
...
- `noTools`: TFA's tool-disable configuration.
...
```ts
const harness = createTfaCodingAgentHarness({
...
prompt creates or changes TFA resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create a TFA extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic TFA adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other TFA configuration:
...
The TFA harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook
