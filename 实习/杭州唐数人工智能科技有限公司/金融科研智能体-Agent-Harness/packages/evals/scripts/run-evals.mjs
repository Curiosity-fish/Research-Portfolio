...
const artifactDirectory = process.env.TFA_EVAL_ARTIFACT_DIR
	? resolve(packageRoot, process.env.TFA_EVAL_ARTIFACT_DIR)
	: resolve(
			packageRoot,
			".eval",
...
if (hasCliModelSelection) {
	...
	provider = process.env.TFA_PROVIDER?.trim() || undefined;
	model = process.env.TFA_MODEL?.trim() || undefined;
	if (Boolean(provider) !== Boolean(model)) {
		console.error("Default model selection requires both TFA_PROVIDER and TFA_MODEL.");
		process.exit(1);
	}
}
...
const childEnvironment = {
	...process.env,
	TFA_EVAL_ARTIFACT_DIR: artifactDirectory,
};
if (provider && model) {
	childEnvironment.TFA_PROVIDER = provider;
	childEnvironment.TFA_MODEL = model;
} else {
	delete childEnvironment.TFA_PROVIDER;
	delete childEnvironment.TFA_MODEL;
}
