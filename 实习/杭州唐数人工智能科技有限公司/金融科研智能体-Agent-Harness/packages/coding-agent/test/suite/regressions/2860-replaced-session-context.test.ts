...
import { fauxAssistantMessage, registerFauxProvider } from "@earendil-works/tfa-ai/compat";
...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `tfa-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const createRuntime: CreateAgentSessionRuntimeFactory = async ({ cwd, sessionManager, sessionStartEvent }) => {
			const services = await createAgentSessionServices({
				...
				resourceLoaderOptions: {
					extensionFactories: [
						(tfa: ExtensionAPI) => {
							tfa.registerProvider(faux.getModel().provider, {
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale tfa/ctx", async () => {
		const events: string[] = [];
		...
		const { runtime } = await createRuntimeForTest(
			(tfa) => {
				...
				tfa.on("session_shutdown", () => {
					events.push(`shutdown:${currentInstance}`);
				});
				tfa.registerCommand("repro", {
					description: "repro",
