import { fauxAssistantMessage, fauxToolCall } from "@earendil-works/tfa-ai";
...
describe("extension active tools next-turn refresh", () => {
	it("applies tfa.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(tfa) => {
				tfa.registerTool({
					...
					execute: async () => {
						tfa.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(tfa) => {
				tfa.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						tfa.setActiveTools([...tfa.getActiveTools(), "after_load"]);
						return {
