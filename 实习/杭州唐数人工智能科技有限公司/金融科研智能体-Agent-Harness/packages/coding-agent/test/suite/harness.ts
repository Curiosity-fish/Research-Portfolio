...
import type { AgentMessage, AgentTool } from "@earendil-works/tfa-agent-core";
...
import type {
	...
	FauxResponseStep,
	Model,
} from "@earendil-works/tfa-ai/compat";
import { registerFauxProvider, streamSimple } from "@earendil-works/tfa-ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `tfa-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
