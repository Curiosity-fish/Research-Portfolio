...
import type { OAuthCredentials } from "@earendil-works/tfa-ai";
import { getModel, streamSimple } from "@earendil-works/tfa-ai/compat";
import { builtinProviders } from "@earendil-works/tfa-ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.tfa/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".tfa", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.tfa/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.tfa/agent/auth.json
 */
...
/** Path to the real tfa agent config directory */
export const TFA_AGENT_DIR = join(homedir(), ".tfa", "agent");

/**
 * Get an AuthStorage instance backed by ~/.tfa/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `tfa-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
