...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "tfa-ext-test-"));
	...
	const extensionCode = `
		export default function(tfa) {
			tfa.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("loads the coding-agent entrypoint without rewriting tfa-ai provider subpaths", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "coding-agent-import.ts"),
			`
				import { getAgentDir } from "@earendil-works/tfa-coding-agent";
				void getAgentDir;
				export default function(tfa) {
					tfa.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only tfa-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@earendil-works/tfa-ai/oauth";
				void oauth;
				export default function(tfa) {
					tfa.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json tfa field", async () => {
	...
	it("keeps package.json tfa extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with tfa field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without tfa field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ tfa: { extensions: ["./entry.ts"] } }));
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(tfa) {
				tfa.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
				tfa.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				tfa.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(tfa) {
				tfa.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(tfa) {
				tfa.on("agent_start", async () => {});
				tfa.on("tool_call", async (event) => undefined);
				tfa.on("agent_end", async () => {});
			}
		`;
