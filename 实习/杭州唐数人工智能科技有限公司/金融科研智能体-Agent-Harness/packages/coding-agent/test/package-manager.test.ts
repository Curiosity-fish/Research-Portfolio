...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.TFA_OFFLINE;
		delete process.env.TFA_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.TFA_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .tfa skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .tfa", async () => {
			const extDir = join(tempDir, ".tfa", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".tfa"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".tfa", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".tfa", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".tfa", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".tfa", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".tfa", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json tfa.extensions in extensions setting", async () => {
			// Create a package with tfa.extensions in package.json
			...
			// Should find the extensions declared in package.json tfa.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .tfa/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-tfa", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-tfa"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-tfa\ndescription: user tfa\n---\n");
		...
		it("should use the project .tfa dir as baseDir for project .tfa skills", async () => {
			const projectBaseDir = join(tempDir, ".tfa");
			const skillPath = join(projectBaseDir, "skills", "project-tfa", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-tfa"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-tfa\ndescription: project tfa\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".tfa", "agent");
		...
		it("should dedupe user skill entries when ~/.tfa/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .tfa auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".tfa\n");

			const skillDir = join(tempDir, ".tfa", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with tfa manifest", async () => {
		...
		it("should keep tfa manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.tfa\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".tfa", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the tfa-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "tfa-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .tfa settings base", () => {
			...
			const rel = relative(join(tempDir, ".tfa"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in tfa manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "tfa-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "tfa-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:tfa-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:tfa-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".tfa"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json tfa.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".tfa", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".tfa", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".tfa", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".tfa", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".tfa", "npm", "node_modules", "example");
