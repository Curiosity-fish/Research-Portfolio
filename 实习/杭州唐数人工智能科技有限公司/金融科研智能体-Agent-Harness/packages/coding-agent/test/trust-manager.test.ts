...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".tfa", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".tfa", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".tfa", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".tfa"), { recursive: true });
			writeFileSync(join(cwd, ".tfa", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".tfa"), { recursive: true, force: true });
