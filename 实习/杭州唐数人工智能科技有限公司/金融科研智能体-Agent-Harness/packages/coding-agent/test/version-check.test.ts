...
const originalSkipVersionCheck = process.env.TFA_SKIP_VERSION_CHECK;
...
afterEach(() => {
	...
	if (originalSkipVersionCheck === undefined) {
		...
		process.env.TFA_SKIP_VERSION_CHECK = originalSkipVersionCheck;
	}
});
...
describe("version checks", () => {
	...
	it("uses the pi.dev version check api with a tfa user agent", async () => {
		...
		expect(fetchMock).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				headers: expect.objectContaining({
					"User-Agent": expect.stringMatching(/^tfa\/1\.2\.3 /),
