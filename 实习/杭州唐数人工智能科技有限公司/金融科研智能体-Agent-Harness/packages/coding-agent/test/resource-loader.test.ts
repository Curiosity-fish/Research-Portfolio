...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "tfa-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".tfa", "prompts");
			...
			const projectSkillDir = join(cwd, ".tfa", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".tfa", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".tfa", "themes"), { recursive: true });
	...
	tfa.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".tfa"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".tfa", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".tfa", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".tfa", "extensions");
...
export default function(tfa) {
	...
	tfa.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(tfa) {
	tfa.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".tfa", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
	...
	tfa.registerCommand("project-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.tfa", async () => {
			const tfaDir = join(cwd, ".tfa");
			mkdirSync(tfaDir, { recursive: true });
			writeFileSync(join(tfaDir, "SYSTEM.md"), "You are a helpful assistant.");
		...
		it("should skip project resources that require trust when project is not trusted", async () => {
			const tfaDir = join(cwd, ".tfa");
			const extensionsDir = join(tfaDir, "extensions");
			const skillDir = join(tfaDir, "skills", "project-skill");
			const promptsDir = join(tfaDir, "prompts");
			const themesDir = join(tfaDir, "themes");
			mkdirSync(extensionsDir, { recursive: true });
			mkdirSync(skillDir, { recursive: true });
			mkdirSync(promptsDir, { recursive: true });
			mkdirSync(themesDir, { recursive: true });
			writeFileSync(join(tfaDir, "SYSTEM.md"), "Project system prompt.");
...
Project skill content`,
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const tfaDir = join(cwd, ".tfa");
			mkdirSync(tfaDir, { recursive: true });
			writeFileSync(join(tfaDir, "APPEND_SYSTEM.md"), "Additional instructions.");
	...
	describe("system prompt sources", () => {
		it("exposes discovered project SYSTEM.md as the system prompt source", async () => {
			const tfaDir = join(cwd, ".tfa");
			const systemPromptPath = join(tfaDir, "SYSTEM.md");
			mkdirSync(tfaDir, { recursive: true });
		...
		it("exposes discovered APPEND_SYSTEM.md as an append system prompt source", async () => {
			const tfaDir = join(cwd, ".tfa");
			const appendSystemPromptPath = join(tfaDir, "APPEND_SYSTEM.md");
			mkdirSync(tfaDir, { recursive: true });
