...
import { getApiProvider, getSupportedThinkingLevels } from "@earendil-works/tfa-ai/compat";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `tfa-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
