...
describe("experimental CLI command composition", () => {
	test("composes tfa command options with the existing parser", () => {
	...
	test.each(["--help", "--version"] as const)("keeps TFA %s handling in existing CLI options", (option) => {
		expect(experimentalCli.parse([option])).toMatchObject({
			ok: true,
			command: { command: "tfa", options: { [option === "--help" ? "help" : "version"]: true } },
		});
	});
	...
	test.each(["tfa", "server", "client"] as const)("executes the parsed %s command", async (name) => {
		const context = {
			runTfa: vi.fn(() => undefined),
			runServer: vi.fn(() => undefined),
			runClient: vi.fn(() => undefined),
		};
		const result = await experimentalCli.execute(name === "tfa" ? [] : [name], context);

		expect(result).toMatchObject({ ok: true, command: { command: name } });
		expect(context.runTfa).toHaveBeenCalledTimes(name === "tfa" ? 1 : 0);
