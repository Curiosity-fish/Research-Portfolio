...
import { getModel } from "@earendil-works/tfa-ai/compat";
...
describe("createAgentSession session manager defaults", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `tfa-sdk-session-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("exposes current session state to the built-in bash tool", async () => {
		...
		const result = await bashTool!.execute("test", {
			command: `printf '%s\\n' "$TFA_SESSION_ID" "$TFA_SESSION_FILE" "$TFA_PROVIDER" "$TFA_MODEL" "$TFA_REASONING_LEVEL"`,
		});
		const output = result.content
			.filter((item): item is { type: "text"; text: string } => item.type === "text")
			.map((item) => item.text)
