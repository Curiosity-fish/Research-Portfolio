...
describe("experimental CLI commands", () => {
	test("selects tfa mode and parses existing CLI arguments", () => {
		expect(
	...
	test("parses a server listener", () => {
		expect(experimentalCli.parse(["server", "--listen", "unix:///tmp/tfa.sock"])).toEqual({
			ok: true,
			command: {
				command: "server",
				listen: [{ transport: "unix", path: "/tmp/tfa.sock" }],
			},
		});
	});

	test("leaves experimental-looking existing option values with the existing parser", () => {
		expect(experimentalCli.parse(["--system-prompt", "--listen", "unix:///tmp/tfa.sock"])).toMatchObject({
			ok: true,
			command: {
				command: "tfa",
				options: { systemPrompt: "--listen", messages: ["unix:///tmp/tfa.sock"] },
			},
		});
	});
	...
	test("stops parsing command options when existing CLI arguments begin", () => {
		...
		expect(result).toMatchObject({
			ok: true,
			command: { command: "tfa", options: { model: "claude-sonnet" } },
		});
		if (!result.ok || result.command.command !== "tfa") return;
	...
	test("parses a client transport address", () => {
		expect(experimentalCli.parse(["client", "--connect", "unix:///tmp/tfa.sock"])).toEqual({
			ok: true,
			command: {
				command: "client",
				connect: { transport: "unix", path: "/tmp/tfa.sock" },
			},
		});
	});
	...
	test.each([[[]], [["server"]], [["client"]]] as const)(
		...
		(argv) => {
			...
			expect(result).toMatchObject({ ok: true, command: { command: argv[0] ?? "tfa" } });
	...
	test("passes unknown options, file arguments, and the positional separator to the existing parser", () => {
		const result = experimentalCli.parse(["--unknown", "@prompt.md", "--", "--listen", "unix:///tmp/tfa.sock"]);
		expect(result).toMatchObject({
			ok: true,
			command: { command: "tfa", options: { fileArgs: ["prompt.md"] } },
		});
		if (!result.ok || result.command.command !== "tfa") return;
		expect(result.command.options.unknownFlags.get("unknown")).toBe(true);
		expect(result.command.options.unknownFlags.get("listen")).toBe("unix:///tmp/tfa.sock");
	});

	test.each([
		[
			["--listen", "unix:///tmp/tfa.sock", "--listen", "unix:///tmp/tfa-admin.sock"],
		...
		[["--listen", "/tmp/tfa.sock"], 'Invalid --listen address "/tmp/tfa.sock"'],
		...
		[["--listen", "unix:///tmp/tfa.sock?wrong=value"], 'Invalid --listen address "unix:///tmp/tfa.sock?wrong=value"'],
		[["--listen", "unix:///tmp/tfa.sock#fragment"], 'Invalid --listen address "unix:///tmp/tfa.sock#fragment"'],
		[["--listen", "unix:/tmp/tfa.sock"], 'Invalid --listen address "unix:/tmp/tfa.sock"'],
		[["--listen", "unix:///tmp/%00pi.sock"], 'Invalid --listen address "unix:///tmp/%00pi.sock"'],
		[
			["client", "--listen", "unix:///tmp/tfa.sock"],
			"The experimental client command does not support existing CLI options yet",
		],
		[
			["server", "--connect", "unix:///tmp/tfa.sock"],
	...
	test("treats command names after the first argument as existing CLI arguments", () => {
		expect(experimentalCli.parse(["--cwd", "/workspace", "server"])).toMatchObject({
			ok: true,
			command: { command: "tfa", options: { messages: ["server"] } },
		});
	});
});
