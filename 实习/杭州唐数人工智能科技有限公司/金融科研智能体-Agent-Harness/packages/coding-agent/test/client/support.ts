import { type ByteTransport, type ByteTransportHandlers, TfaClient } from "@earendil-works/tfa-client";
import {
	type ClientMessage,
...
export async function connectClient(server: MemoryServer): Promise<TfaClient> {
	const client = new TfaClient({ transportFactory: (handlers) => server.connect(handlers) });
