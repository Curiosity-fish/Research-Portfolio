...
Extensions can register custom model providers via `tfa.registerProvider()`. This enables:
...
Extensions can register either a complete tfa-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. TFA composes `models.json` overrides above registered native providers.
...
export default function (tfa: ExtensionAPI) {
  tfa.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  tfa.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. tfa waits for the factory before startup continues, so the provider is available during interactive startup and to `tfa --list-models`.
...
Use `tfa.unregisterProvider(name)` to remove a provider that was previously registered via `tfa.registerProvider(name, ...)`:
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map tfa levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
tfa.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@earendil-works/tfa-ai";
...
Credentials are persisted in `~/.tfa/agent/auth.json`:
...
When a request exceeds the model's context window, tfa can recover automatically by compacting the conversation and retrying. This recovery only kicks in if tfa recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of tfa's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/earendil-works/pi-mono/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message tfa does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase tfa recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before tfa tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what tfa checks. With this in place, tfa will:
...
- Match a provider-specific pattern, not tfa's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of tfa's normal retry-with-backoff path.
...
interface ProviderModelConfig {
  ...
  /** Maps tfa thinking levels to provider/model-specific values; null marks a level unsupported. */
