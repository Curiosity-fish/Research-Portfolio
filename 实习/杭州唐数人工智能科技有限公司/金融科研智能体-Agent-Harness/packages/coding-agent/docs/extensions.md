> tfa can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend tfa's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.tfa/agent/extensions/` (global) or `.tfa/extensions/` (project-local) for auto-discovery. Use `tfa -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `tfa.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `tfa.registerCommand()`
- **Session persistence** - Store state that survives restarts via `tfa.appendEntry()`
...
Create `~/.tfa/agent/extensions/my-extension.ts`:
...
export default function (tfa: ExtensionAPI) {
  ...
  tfa.on("tool_call", async (event, ctx) => {
...
Extensions are auto-discovered from trusted locations. Project-local `.tfa/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.tfa/agent/extensions/*.ts` | Global (all projects) |
| `~/.tfa/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.tfa/extensions/*.ts` | Project-local |
| `.tfa/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as tfa packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@earendil-works/tfa-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@earendil-works/tfa-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@earendil-works/tfa-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed tfa packages installed with `tfa install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (tfa: ExtensionAPI) {
  // Subscribe to events
  tfa.on("event_name", async (event, ctx) => {
  ...
  tfa.registerTool({ ... });
  tfa.registerCommand("name", { ... });
  tfa.registerShortcut("ctrl+x", { ... });
  tfa.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, tfa awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `tfa.registerProvider()` are flushed.
...
export default async function (tfa: ExtensionAPI) {
  ...
  tfa.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `tfa --list-models`.
...
thinking level changes (settings, keybinding, tfa.setThinkingLevel())
...
Fired before tfa decides whether to trust a project with dynamic configs (`.tfa` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
tfa.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether tfa asks, trusts, or declines by default.
...
```typescript
tfa.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
tfa.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `tfa.setSessionName()`.

```typescript
tfa.on("session_info_changed", async (event, ctx) => {
...
```typescript
tfa.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, tfa emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
tfa.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, tfa emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
```typescript
tfa.on("session_before_compact", async (event, ctx) => {
...
tfa.on("session_compact", async (event, ctx) => {
...
```typescript
tfa.on("session_before_tree", async (event, ctx) => {
...
tfa.on("session_tree", async (event, ctx) => {
...
```typescript
tfa.on("session_shutdown", async (event, ctx) => {
...
```typescript
tfa.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data TFA uses to build the system prompt. This lets you inspect what TFA has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but TFA may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know TFA will not continue running automatically.

```typescript
tfa.on("agent_start", async (_event, ctx) => {});

tfa.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

tfa.on("agent_settled", async (_event, ctx) => {
...
```typescript
tfa.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

tfa.on("turn_end", async (event, ctx) => {
...
```typescript
tfa.on("message_start", async (event, ctx) => {
  // event.message
});

tfa.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

tfa.on("message_end", async (event, ctx) => {
...
```typescript
tfa.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

tfa.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

tfa.on("tool_execution_end", async (event, ctx) => {
...
```typescript
tfa.on("context", async (event, ctx) => {
...
```typescript
tfa.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header tfa adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports TFA's system prompt string rather than the final serialized provider payload.

```typescript
tfa.on("before_provider_request", (event, ctx) => {
...
```typescript
tfa.on("after_provider_response", (event, ctx) => {
...
```typescript
tfa.on("model_select", async (event, ctx) => {
...
```typescript
tfa.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `tfa.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, tfa waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@earendil-works/tfa-coding-agent";
...
```typescript
import { isBashToolResult } from "@earendil-works/tfa-coding-agent";

tfa.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@earendil-works/tfa-coding-agent";

tfa.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap tfa's built-in local bash backend
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.tfa` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@earendil-works/tfa-coding-agent";
import { join } from "node:path";

export default function (tfa: ExtensionAPI) {
  tfa.on("session_start", (_event, ctx) => {
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective tfa-ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while tfa is idle.
...
Control flow helpers. `ctx.isIdle()` is false while TFA is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of tfa.
...
Returns TFA's current system prompt string.
...
```typescript
tfa.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs TFA currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `tfa` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@earendil-works/tfa-coding-agent";

tfa.registerCommand("switch", {
...
- Captured old `tfa` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
tfa.registerCommand("reload-runtime", {
...
export default function (tfa: ExtensionAPI) {
  ...
  tfa.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      tfa.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### tfa.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### tfa.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`tfa.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `tfa.getAllTools()` and are callable by the LLM without `/reload`.

Use `tfa.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@earendil-works/tfa-ai";
...
### tfa.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`tfa.appendEntry()`](#piappendentrycustomtype-data) with [`tfa.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### tfa.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
tfa.sendUserMessage("What is 2+2?");
...
tfa.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
tfa.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### tfa.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `tfa.registerEntryRenderer()`.

```typescript
tfa.appendEntry("my-state", { count: 42 });
tfa.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### tfa.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
tfa.setSessionName("Refactor auth module");
```
...
```typescript
const name = tfa.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### tfa.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
tfa.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
tfa.setLabel(entryId, undefined);
...
### tfa.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, tfa keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
tfa.registerCommand("stats", {
...
```typescript
import type { AutocompleteItem } from "@earendil-works/tfa-tui";

tfa.registerCommand("deploy", {
...
```typescript
const commands = tfa.getCommands();
...
### tfa.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `tfa.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### tfa.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, TFA renders the transformed content with its built-in renderer.
...
```typescript
tfa.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
  if (isStreaming || messageType === "assistant-thinking") return markdown;
  return markdown.replaceAll("-->", "→");
});
```

If a transformer throws, TFA keeps the Markdown produced so far and continues with the next transformer. The hook is display-only: the original message remains unchanged in the session and model context. It runs for new user messages, assistant streaming updates, restored session messages, and terminal width changes, so transformers should remain synchronous and inexpensive.

### tfa.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `tfa.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@earendil-works/tfa-tui";

tfa.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### tfa.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
tfa.registerShortcut("ctrl+shift+p", {
...
### tfa.registerFlag(name, options)

Register a CLI flag.

```typescript
tfa.registerFlag("plan", {
...
### tfa.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await tfa.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### tfa.getActiveTools() / tfa.getAllTools() / tfa.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `tfa.getActiveTools()` returns the active tool names as `string[]`; `tfa.getAllTools()` returns metadata for all configured tools.

```typescript
const active = tfa.getActiveTools(); // ["read", "bash", ...]
const all = tfa.getAllTools();
// all = [{
//   name: "read",
...
tfa.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
tfa.setActiveTools(["read", "bash"]); // Switch to read-only
```

`tfa.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await tfa.setModel(model);
  if (!success) {
...
### tfa.getThinkingLevel() / tfa.setThinkingLevel(level)
...
```typescript
const current = tfa.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
tfa.setThinkingLevel("high");
```
...
```typescript
tfa.events.on("my:event", (data) => { ... });
tfa.events.emit("my:event", { ... });
```

### tfa.registerProvider(name, config)
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@earendil-works/tfa-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@earendil-works/tfa-ai";
...
tfa.registerProvider(provider);

// Register a new provider with custom models
tfa.registerProvider("my-proxy", {
  name: "My Proxy",
...
tfa.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
tfa.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
tfa.registerProvider("corporate-ai", {
...
The object form accepts a complete tfa-ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
tfa.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    tfa.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `tfa.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `tfa.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@earendil-works/tfa-coding-agent";
...
import { Type } from "typebox";
import { StringEnum } from "@earendil-works/tfa-ai";
import { Text } from "@earendil-works/tfa-tui";
...
tfa.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via tfa.exec (captured from extension closure)
    const result = await tfa.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. TFA persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@earendil-works/tfa-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when tfa resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
tfa -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
tfa --no-builtin-tools -e ./my-extension.ts
```
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@earendil-works/tfa-coding-agent";
...
For `user_bash`, extensions can reuse tfa's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.

The bash tool also supports a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool } from "@earendil-works/tfa-coding-agent";
...
export default function (tfa: ExtensionAPI) {
  ...
  tfa.registerTool({ name: "db_connect", ... });
  tfa.registerTool({ name: "db_query", ... });
  tfa.registerTool({ name: "db_close", ... });

  tfa.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
```typescript
import { keyHint } from "@earendil-works/tfa-coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `tfa.setActiveTools()` during execution. TFA detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `tfa.registerTool()` so it appears in `tfa.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `tfa.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. TFA records which tools were added on the loader's tool result.
5. Before the next model response, TFA exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `tfa.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** TFA adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: TFA sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

TFA also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (tfa: ExtensionAPI) {
  ...
  tfa.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const active = tfa.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      tfa.setActiveTools([...new Set([...active, ...added])]);
  ...
  tfa.on("session_start", () => {
    // Keep searchable tools registered but initially inactive. Preserve built-ins
    // and tools owned by other extensions, and keep the loader itself active.
    const initialTools = tfa.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    tfa.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("tfa - my-project");
...
```typescript
import { Text, Component } from "@earendil-works/tfa-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@earendil-works/tfa-coding-agent";
import { matchesKey } from "@earendil-works/tfa-tui";
...
tfa.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `tfa.sendMessage()`:
...
```typescript
tfa.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

tfa.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@earendil-works/tfa-coding-agent";
...
| `event-bus.ts` | Inter-extension events | `tfa.events` |
| **Session Metadata** |||
