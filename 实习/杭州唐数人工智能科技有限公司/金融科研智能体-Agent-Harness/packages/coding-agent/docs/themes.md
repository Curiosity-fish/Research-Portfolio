> tfa can create themes. Ask it to build one for your setup.
...
TFA loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.tfa/agent/themes/*.json`
- Project: `.tfa/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `tfa.themes` entries in `package.json`
...
On first run, tfa detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.tfa/agent/themes
vim ~/.tfa/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, tfa reloads it automatically for immediate visual feedback.
...
TFA uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, tfa falls back to the nearest approximation.
