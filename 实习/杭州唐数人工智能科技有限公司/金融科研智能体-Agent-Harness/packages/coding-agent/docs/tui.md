> tfa can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@earendil-works/tfa-tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@earendil-works/tfa-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `TFA_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@earendil-works/tfa-tui";
...
Import from `@earendil-works/tfa-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@earendil-works/tfa-tui";
```
...
```typescript
import { matchesKey, Key } from "@earendil-works/tfa-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@earendil-works/tfa-tui";
...
```typescript
import { getMarkdownTheme } from "@earendil-works/tfa-coding-agent";
import { Markdown } from "@earendil-works/tfa-tui";
...
Set `TFA_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
TFA_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@earendil-works/tfa-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@earendil-works/tfa-coding-agent";
import { DynamicBorder } from "@earendil-works/tfa-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@earendil-works/tfa-tui";
...
```typescript
import { BorderedLoader } from "@earendil-works/tfa-coding-agent";

tfa.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@earendil-works/tfa-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@earendil-works/tfa-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@earendil-works/tfa-tui";

tfa.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while tfa is streaming a response.
...
import { matchesKey, truncateToWidth } from "@earendil-works/tfa-tui";
