...
```
~/.tfa/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.tfa/agent/sessions/`.

TFA also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, tfa uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([tfa-mono](https://github.com/earendil-works/pi-mono)):
...
For TypeScript definitions in your project, inspect `node_modules/@earendil-works/tfa-coding-agent/dist/` and `node_modules/@earendil-works/tfa-ai/dist/`.
...
### Base Message Types (from tfa-ai)
...
The exported tfa-ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before tfa persists the assistant message, so `"pending"` should never appear in session JSONL.

### Extended Message Types (from tfa-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if tfa-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `tfa.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `tfa.setSessionName()` in extensions.
