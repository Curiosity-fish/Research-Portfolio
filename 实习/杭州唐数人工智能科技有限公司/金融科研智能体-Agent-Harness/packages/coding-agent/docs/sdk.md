> tfa can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to tfa's agent capabilities. Use it to embed tfa in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @earendil-works/tfa-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@earendil-works/tfa-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `tfa.sendMessage()`.
...
The `Agent` class (from `@earendil-works/tfa-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.tfa/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.tfa/extensions/`)
- Project skills:
  - `.tfa/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.tfa/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.tfa/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@earendil-works/tfa-ai";
import { ModelRuntime } from "@earendil-works/tfa-coding-agent";
...
```typescript
import { InMemoryCredentialStore } from "@earendil-works/tfa-ai";
import { createAgentSession, ModelRuntime } from "@earendil-works/tfa-coding-agent";

// Default: uses ~/.tfa/agent/auth.json and ~/.tfa/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any tfa-ai CredentialStore
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@earendil-works/tfa-coding-agent";
...
The `edit` tool returns `details.diff` for TFA's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@earendil-works/tfa-coding-agent";
...
import { createAgentSession, defineTool } from "@earendil-works/tfa-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `tfa.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `tfa.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.tfa/agent/extensions/`, `.tfa/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@earendil-works/tfa-coding-agent";
...
**Event Bus:** Extensions can communicate via `tfa.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@earendil-works/tfa-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@earendil-works/tfa-coding-agent";
...
1. Global: `~/.tfa/agent/settings.json`
2. Project: `<cwd>/.tfa/settings.json`
...
// Auth and Models
ModelRuntime // implements tfa-ai Models and owns credential storage
