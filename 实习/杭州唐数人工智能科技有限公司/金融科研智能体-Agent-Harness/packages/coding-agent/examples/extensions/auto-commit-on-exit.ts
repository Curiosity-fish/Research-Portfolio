...
export default function (tfa: ExtensionAPI) {
	tfa.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await tfa.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[tfa] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await tfa.exec("git", ["add", "-A"]);
		const { code: commitCode } = await tfa.exec("git", ["commit", "-m", commitMessage]);
