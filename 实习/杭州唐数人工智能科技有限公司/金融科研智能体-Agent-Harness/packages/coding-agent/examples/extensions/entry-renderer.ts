 ...
 * to the LLM. Custom entries are stored in the session via tfa.appendEntry() and
 * rendered in interactive mode via tfa.registerEntryRenderer().
 *
...
export default function (tfa: ExtensionAPI) {
	tfa.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	tfa.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			tfa.appendEntry<StatusCardData>("status-card", {
