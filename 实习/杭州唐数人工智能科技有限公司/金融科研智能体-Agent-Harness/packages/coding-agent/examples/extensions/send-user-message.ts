 ...
 *
 * Demonstrates tfa.sendUserMessage() for sending user messages from extensions.
 * Unlike tfa.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (tfa: ExtensionAPI) {
	...
	tfa.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			tfa.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	tfa.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				tfa.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	tfa.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				tfa.sendUserMessage(args, { deliverAs: "followUp" });
