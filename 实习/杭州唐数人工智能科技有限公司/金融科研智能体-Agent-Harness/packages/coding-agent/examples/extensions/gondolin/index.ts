 ...
 *
 * Runs tfa's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   tfa -e /path/to/tfa/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (tfa: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `tfa ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	tfa.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
