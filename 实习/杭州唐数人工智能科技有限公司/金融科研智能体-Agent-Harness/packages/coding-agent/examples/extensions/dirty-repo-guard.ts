...
async function checkDirtyRepo(
	...
	const { stdout, code } = await tfa.exec("git", ["status", "--porcelain"]);
...
export default function (tfa: ExtensionAPI) {
	tfa.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(tfa, ctx, action);
	});

	tfa.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(tfa, ctx, "fork");
	});
}
