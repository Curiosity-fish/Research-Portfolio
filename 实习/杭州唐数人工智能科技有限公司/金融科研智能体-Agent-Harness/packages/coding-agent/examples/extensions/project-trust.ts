 ...
 *
 *   mkdir -p ~/.tfa/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.tfa/agent/extensions/
 *
 * Or:
 *
 *   tfa -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .tfa, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@earendil-works/tfa-coding-agent";
...
export default function (tfa: ExtensionAPI) {
	...
	tfa.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
