...
Example extensions for tfa-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
tfa --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.tfa/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `tfa.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs tfa theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `tfa.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using tfa-ai's built-in Anthropic/OpenAI streaming via proxy |
