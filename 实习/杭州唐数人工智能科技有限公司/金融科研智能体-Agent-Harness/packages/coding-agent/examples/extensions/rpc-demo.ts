...
export default function (tfa: ExtensionAPI) {
	...
	tfa.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "tfa RPC Demo (new session)" : "tfa RPC Demo");
	...
	tfa.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	tfa.on("turn_end", async (_event, ctx) => {
	...
	tfa.registerCommand("rpc-editor", {
	...
	tfa.registerCommand("rpc-prefill", {
