 ...
 *
 * Usage:
 *   tfa -e ./ssh.ts --ssh user@host
 *   tfa -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (tfa: ExtensionAPI) {
	tfa.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	tfa.on("session_start", async (_event, ctx) => {
		...
		const arg = tfa.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
