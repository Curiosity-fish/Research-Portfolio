 ...
 * - ~/.tfa/agent/presets.json (global)
 * - <cwd>/.tfa/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `tfa --preset plan` - start with plan preset
...
import type { Api, Model } from "@earendil-works/tfa-ai";
import type { ExtensionAPI, ExtensionContext } from "@earendil-works/tfa-coding-agent";
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@earendil-works/tfa-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@earendil-works/tfa-tui";
...
export default function presetExtension(tfa: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: tfa.getThinkingLevel(),
				tools: tfa.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			tfa.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = tfa.getAllTools().map((t) => t.name);
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await tfa.setModel(originalState.model);
				}
				tfa.setThinkingLevel(originalState.thinkingLevel);
				tfa.setActiveTools(originalState.tools);
			} else {
				tfa.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	tfa.registerShortcut(Key.ctrlShift("u"), {
	...
	tfa.on("before_agent_start", async (event) => {
	...
	tfa.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = tfa.getFlag("preset");
	...
	tfa.on("turn_start", async () => {
		if (activePresetName) {
			tfa.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
