...
async function resolveGitHubRepo(tfa: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await tfa.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (tfa: ExtensionAPI): void {
	tfa.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(tfa, ctx.cwd);
		if (!resolvedRepo.ok) {
