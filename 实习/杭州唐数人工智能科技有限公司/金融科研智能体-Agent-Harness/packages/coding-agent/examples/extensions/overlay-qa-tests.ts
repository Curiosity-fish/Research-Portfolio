 ...
 *
 * Usage: tfa --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@earendil-works/tfa-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@earendil-works/tfa-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/tfa-tui";
...
export default function (tfa: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like tfa-doom would need)
	tfa.registerCommand("overlay-animation", {
	...
	tfa.registerCommand("overlay-anchors", {
	...
	tfa.registerCommand("overlay-margins", {
	...
	tfa.registerCommand("overlay-stack", {
	...
	tfa.registerCommand("overlay-overflow", {
	...
	tfa.registerCommand("overlay-edge", {
	...
	tfa.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	tfa.registerCommand("overlay-maxheight", {
	...
	tfa.registerCommand("overlay-sidepanel", {
	...
	tfa.registerCommand("overlay-toggle", {
	...
	tfa.registerCommand("overlay-passive", {
	...
	tfa.registerCommand("overlay-focus", {
	...
	tfa.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/tfa-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/tfa-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like tfa-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (tfa-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
