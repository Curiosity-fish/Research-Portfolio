...
import { readTfaManifest, type TfaManifest } from "./tfa-manifest.ts";
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.TFA_OFFLINE;
...
type SkillDiscoveryMode = "tfa" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "tfa" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "tfa");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside tfa and resolve tfa APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @earendil-works/tfa-* peers. Stale auto-installed tfa peers can otherwise block updates.
	...
	private getGitUpdateMarkerPath(targetDir: string): string {
		return join(dirname(targetDir), `.${basename(targetDir)}.tfa-update-incomplete`);
	}
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "tfa-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = readTfaManifest(join(packageRoot, "package.json"));
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof TfaManifest];
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			...
			// Project skills from .tfa/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "tfa"),
				projectMetadata,
		...
		// User extensions from ~/.tfa/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.tfa/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "tfa"),
			userMetadata,
