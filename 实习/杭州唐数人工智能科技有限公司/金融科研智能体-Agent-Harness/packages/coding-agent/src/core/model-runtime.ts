...
import * as builtinProviderCatalog from "@earendil-works/tfa-ai/providers/all";
...
/** Configured tfa-ai Models collection used by coding-agent and SDK consumers. */
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const runtime = new ModelRuntime(
			...
			modelsStore,
			providers,
			process.env.TFA_OFFLINE === undefined,
		);
	...
	async refresh(options: ModelsRefreshOptions = {}): Promise<ModelsRefreshResult> {
		...
		// Published tfa-ai builds before ModelsStore returned void and accepted a provider ID.
