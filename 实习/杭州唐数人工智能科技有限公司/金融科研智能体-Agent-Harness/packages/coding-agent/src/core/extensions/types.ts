...
import type {
	...
	ThinkingLevel,
	ToolExecutionMode,
} from "@earendil-works/tfa-agent-core";
import type {
	Api,
...
export interface ExtensionUIContext {
	...
	/** Request approval for a high-risk tool operation (request_approval mode). Optional; not all modes implement it. */
	askApproval?(
		input: { tool: string; target: string; risk: string },
	 ...
	 * extend `CustomEditor` from `@earendil-works/tfa-coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@earendil-works/tfa-coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown tfa and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what TFA loaded without re-discovering resources. */
...
export interface ExtensionAPI {
	...
	/** Register a transformer for user and assistant Markdown before TFA renders it in the interactive transcript. */
...
/** Configuration for registering a provider via tfa.registerProvider(). */
...
export type ExtensionFactory = (tfa: ExtensionAPI) => void | Promise<void>;
...
export interface ExtensionRuntimeState {
	...
	/** Native tfa-ai provider registrations queued during extension loading, processed when runner binds. */
...
/**
 * Action implementations for tfa.* API methods.
