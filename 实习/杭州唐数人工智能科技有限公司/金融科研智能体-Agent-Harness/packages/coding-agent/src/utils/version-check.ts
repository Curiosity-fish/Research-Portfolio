...
import { getTfaUserAgent } from "./tfa-user-agent.ts";
...
export async function getLatestPiRelease(
	...
	if (process.env.TFA_OFFLINE) return undefined;

	const response = await fetchWithRetry(
		LATEST_VERSION_URL,
		{
			headers: {
				"User-Agent": getTfaUserAgent(currentVersion),
...
export async function checkForNewPiVersion(currentVersion: string): Promise<LatestPiRelease | undefined> {
	if (process.env.TFA_SKIP_VERSION_CHECK) return undefined;
