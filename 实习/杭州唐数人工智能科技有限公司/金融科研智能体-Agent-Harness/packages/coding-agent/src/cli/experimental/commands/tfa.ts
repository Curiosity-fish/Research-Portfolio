...
export interface TfaCommand {
	readonly command: "tfa";
	readonly auth?: AuthInput;
	readonly options: Args;
	readonly listen?: readonly TransportAddress[];
}

export interface TfaCommandContext {
	runTfa(command: TfaCommand): void | Promise<void>;
}

const listenOption = transportOption("--listen");

export const tfaCommand = new Command<TfaCommand, TfaCommandContext>("tfa")
	.option(listenOption)
	...
	.action((command, context) => context.runTfa(command));
