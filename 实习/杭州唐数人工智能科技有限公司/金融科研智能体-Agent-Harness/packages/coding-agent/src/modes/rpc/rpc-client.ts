...
import type { AgentMessage, ThinkingLevel } from "@earendil-works/tfa-agent-core";
import type { ImageContent } from "@earendil-works/tfa-ai";
...
export class RpcClient {
	...
	/**
	 * Subscribe to approval requests (request_approval 权限模式)。
	 * 返回退订函数。
	 */
	onApproval(listener: (request: RpcApprovalRequest) => void): () => void {
		return this.onEvent((event) => {
			const candidate = event as { type?: string } & Partial<RpcApprovalRequest>;
			if (candidate.type === "approval_request") {
				listener(candidate as RpcApprovalRequest);
			}
		});
	}

	/**
	 * Respond to an approval request (只作用于对应 request id)。
	 */
	async respondApproval(id: string, approved: boolean): Promise<void> {
		await this.sendRaw({ type: "approval_response", id, approved });
	}
	...
	private async sendRaw(message: RpcApprovalResponse): Promise<void> {
