...
import { type ImageContent, modelsAreEqual } from "@earendil-works/tfa-ai";
...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "tfa -ne".';
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.TFA_OFFLINE);
	if (offlineMode) {
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `tfa update` drain naturally instead.
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.TFA_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: TFA_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
