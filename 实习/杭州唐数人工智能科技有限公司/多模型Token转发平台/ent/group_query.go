...
func (_q *GroupQuery) QueryGroupPlatforms() *GroupPlatformQuery {
...
func (_q *GroupQuery) WithGroupPlatforms(opts ...func(*GroupPlatformQuery)) *GroupQuery {
	query := (&GroupPlatformClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withGroupPlatforms; query != nil {
		if err := _q.loadGroupPlatforms(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupPlatforms = []*GroupPlatform{} },
			func(n *Group, e *GroupPlatform) { n.Edges.GroupPlatforms = append(n.Edges.GroupPlatforms, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadGroupPlatforms(ctx context.Context, query *GroupPlatformQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupPlatform)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupplatform.FieldGroupID)
	}
	query.Where(predicate.GroupPlatform(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupPlatformsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
